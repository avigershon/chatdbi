import { executeSql , type ExecuteSqlForm} from './snowflake.connect';
...
// Helper function to determine if code is SQL or Python
function detectCodeType(code: string): 'python' | 'sql' {
	const trimmedCode = code.trim().toUpperCase();
	return trimmedCode.startsWith("SELECT") || trimmedCode.startsWith("WITH") ? 'sql' : 'python';
}
...
self.onmessage = async (event) => {
	...
	matplotlib.pyplot.show = show`);
		...
		// Execute SQL code by sending it to the backend
		try {
			const executeSqlForm: ExecuteSqlForm = {
				sql: code
			  };

			const result = await executeSql(executeSqlForm);
